...
CREATE OR REPLACE WAREHOUSE tb_de_wh
    WAREHOUSE_SIZE = 'large' -- 初期データロード用にLargeサイズ - このスクリプトの最後でXSmallにスケールダウン
...
-- year、make、modelからオブジェクトを構築し、truck_build カラムに格納
...
-- truck_build オブジェクト内のmakeデータを意図的に破損させる
UPDATE tb_101.raw_pos.truck
